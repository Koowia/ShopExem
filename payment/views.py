...
def create_stripe_checkout_session(orders, request):
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            ...
            cancel_url=request.build_absolute_uri('/payment/stripe/cancel/') + f'?order_id={orders.id}',
            metadata={
                'order_id': orders.id,
            }
        )
        orders.stripe_payment_intent_id = checkout_session.payment_intent 
...
def stripe_success(request):
    ...
    try:
        ...
        return render(request, 'payment/stripe_error.html', context, status=500)
